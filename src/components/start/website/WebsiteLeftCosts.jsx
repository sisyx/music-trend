...
import { removeSite } from "@/Redux/core/cart/cartSlice";

function WebsiteLeftCosts({ costs = [] }) {
            ...
            <>
                {costs.map(cost =>
                    <div className={`relative flex flex-col items-center rounded-md w-full bg-gray-200 hover:bg-gray-300 cursor-pointer md:gap-3 ${styles.cart_influ}`}>
                        <IconButton onClick={() => handleRemove(cost.site)} sx={{position: "absolute", top: "0rem", left: "0rem"}}>
                            {/* <Delete sx={{color: "#a00"}} /> */}
                            <IoCloseSharp />
                        </IconButton>
                        <div className="flex items-start justify-end w-full gap-2 p-2 mt-6">
                            ...
                            <div className="flex flex-col gap-2 items-center justify-center">
                                <span>
                                    <img className={`max-w-16 w-16 rounded-full overflow-hidden aspect-square ${styles.cart_influ_img}`} src={cost?.siteImg || "/logo.png"} alt="Profile Photo" />
                                </span>
                            </div>
                        </div>
                    </div>
...
export default WebsiteLeftCosts;