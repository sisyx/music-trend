...
function Timeline({ data, className = "" }) {
    return ( 
        <ol className={`items-center sm:flex ${className}`}>
            {
                data.map((item, index) => 
                    <li className={`relative mb-6 sm:mb-0 min-w-48 w-full flex flex-col items-center justify-center ${styles.item}`} style={{
                        ...
                        <div className="w-full flex items-center">
                            <div className="z-10 flex items-center justify-center w-6 h-6 bg-blue-100 rounded-full ring-0 ring-white shrink-0 ">
                                {
                                    item.active ? 
                                    <IoCheckmarkDoneCircle className="text-telegram" />
                                    :
